...
public class TransportationProblem {
                ...
                for (int c = 0; c < demand.length; c++) {
                    if (matrix[r][c] == null) {
                        ...
                        if (getClosedPath(dummy).length == 0) {
                            matrix[r][c] = dummy;
                            System.out.println("Установим 0 в клетку a" + (r+1) + "b" + (c+1));
                            fixDegenerateCase();
                            return;
                        }
                    }
    ...
    public static void printSummary() {
        for (int r = 0; r < supply.length - 1; r++) {
            for (int c = 0; c < demand.length; c++) {
                if (matrix[r][c] != null) {
                    System.out.println("Из " + (r + 1) + "-го района производства направим " + (int) matrix[r][c].quantity + " ед. груза в " + (c + 1) + " пункт потребления");
                }
            }
        }
        for (int c = 0; c < demand.length; c++) {
            if (matrix[supply.length - 1][c] != null) {
                System.out.println("Суммарный ущерб " + (c + 1) + "-го пункта потребления из-за невыполнения заявки равен " + matrix[supply.length - 1][c].quantity * matrix[supply.length - 1][c].costPerUnit +
                        ", количество недопоставленого груза: " + (int) matrix[supply.length - 1][c].quantity);
            }
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        init(filename);
        printSourceData();
        System.out.println("Найдем опорный план методом северо-западного угла");
        northWestCornerRule();
